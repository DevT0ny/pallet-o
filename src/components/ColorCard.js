    ...
    return (
        ...
        >
            ...
            <div className="w-full h-full opacity-0 transition-opacity duration-200 ease-in hover:opacity-50 bg-black  absolute top-0 left-0 flex flex-row justify-around " >
                <i className="icon p-5 inline-block rounded-full text-lg my-auto bg-center" onClick={ ()=>log(color,id) } style={{backgroundImage:`url(${plus})` ,backgroundRepeat:"no-repeat"   }}> </i>
                <i className={"icon p-5 inline-block rounded-full text-lg my-auto bg-center  "+copyClassName} onClick={ ()=>showPopup(true)}
                     data-clipboard-action="copy" data-clipboard-target={'#'+copyId}
                    style={{backgroundImage:`url(${copy})` ,backgroundRepeat:"no-repeat"   }}> </i>
                  <i className="icon p-5 inline-block rounded-full text-lg my-auto bg-center" onClick={ ()=>deleteColor(id) } style={{backgroundImage:`url(${trash})` ,backgroundRepeat:"no-repeat"    }}> </i>
            </div>
        </div>
    )
}
